...
(function(){
  ...
  controlsTpl = ''+
    '<div class="proper-commands">'+
      '<a href="#" title="Emphasis (CTRL+SHIFT+E)" class="command em icon-24 icon-italic" command="em"><div>Emphasis</div></a>'+
      '<a href="#" title="Strong (CTRL+SHIFT+S)" class="command strong icon-24 icon-bold" command="strong"><div>Strong</div></a>' +
      '<a title="Link (CTRL+SHIFT+L)" href="#" class="command link icon-24 icon-link" command="link"><div>Link</div></a>' +
      '<a href="#" title="Bullet List (CTRL+SHIFT+B)" class="command ul icon-24 icon-list-ul" command="ul"><div>Bullets List</div></a>' +
      '<a href="#" title="Numbered List (CTRL+SHIFT+N)" class="command ol icon-24 icon-list-ol" command="ol"><div>Numbered List</div></a>' +
      '<br class="clear"/>' +
    '</div>';
  ...
  this.Proper = function(options) {
    ...
    var commands = {
      ...
      code: {
        ...
        toggleOff: function () {
          var sel;
          if ('webkitRequestAnimationFrame' in window && (sel = saveSelection()).collapsed) {
      ...
      link: {
        exec: function(cnode) {
          ...
          if( cnode ){
            var l = $(cnode);
            curr = l.attr('href') || "http://";
          }
          removeFormat();
          curr = window.prompt('URL:', curr);
          if( $.trim(curr) == '' ) document.execCommand('unlink', false, true);
          else document.execCommand('createLink', false, curr);
        }
      },
      // link: {
      //   exec: function() {
      ...
      $(el).bind('paste', function () {
        ...
        getPastedContent(function (node) {
          ...
          setTimeout(function(){
            ...
            $(activeElement).trigger('changed.editor');
          }, 500);
        });
      });
    ...
    function activate (el, opts) {
      ...
      if (options.markup) {
        ...
        $controls.prependTo($(options.controlsTarget));
      }
      ...
      $('.proper-commands a.command').click(function(e) {
        ...
        if( rng.startContainer.parentNode ){
          pnode = $(rng.startContainer.parentNode);
        }
        exec($(e.currentTarget).attr('command'), pnode);
        updateCommandState();
    ...
    return {
      ...
      on:      function () { events.bind.apply(events, arguments); },
      unbind:  function () { events.unbind.apply(events, arguments); },
      off:     function () { events.unbind.apply(events, arguments); },
